...
public class Ecosystem {
	...
	private static ArrayList<Organism> bOrgs;
	private static ArrayList<Organism> rOrgs;
	private static ArrayList<Organism> yOrgs;
	...
	public static void init() {
		...
		for (int i = 0; i < amount[0]; i++) {
			...
			energy -= Organism.lifeForces[Organism.GREEN];
		}
		for (int i = 0; i < amount[1]; i++) {
			bOrgs.add(new OrgBlue());
            energy -= Organism.lifeForces[Organism.BLUE];
		}
		for (int i = 0; i < amount[2]; i++) {
			rOrgs.add(new OrgRed());
            energy -= Organism.lifeForces[Organism.RED];
		}
	...
	public static void updateGreen() {
		while (energy > Organism.lifeForces[Organism.GREEN]) {
			gOrgs.add(new OrgGreen());
			energy -= Organism.lifeForces[Organism.GREEN];
		}
	}
